...
def run_local(args: argparse.Namespace, rose_tree: RoseTree, cachedir: Optional[str], cwl_runner: str,
    ...
    if cwl_runner == 'toil-cwl-runner':
        ...
        cmd += ['--outdir', f'outdir_toil_{yaml_stem}_{date_time}',
                ...
                '--clean', 'never',  # This effectively disables caching, but is reproducible
                ...
                # '--logFile', 'logfile',
                # '--restart',
                # Despite setting the work directory, some temporary files are still being written to the /tmp folder.
                # Additionally, another log file is being created in the /tmp folder. To prevent this, you need to set the TMPDIR environment variable.
                # https://github.com/DataBiosphere/toil/issues/3371
                '--tmp-outdir-prefix', 'tmpdir/tmp',
                '--tmpdir-prefix', 'tmpdir/tmp',
