...
def run_local(args: argparse.Namespace, rose_tree: RoseTree, cachedir: Optional[str], cwl_runner: str,
    ...
    if cwl_runner == 'toil-cwl-runner':
        ...
        cmd += ['--outdir', 'outdir_toil',
                ...
                '--coordinationDir', '/data1',  # "Absolute path to directory where Toil will keep state and lock files." it should be non NFS
                # '--log-dir', 'logdir',
                # '--logFile', 'logfile',
                # '--restart',
                '--disableCaching',
                ...
                # '--logLevel', 'DEBUG' # Log at given level
                '--cleanWorkDir', 'never',
                '--clean', 'never',
                f'autogenerated/{yaml_stem}.cwl', f'autogenerated/{yaml_stem}_inputs.yml']
        cmdline = ' '.join(cmd)
