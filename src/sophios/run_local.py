...
def run_local(args: argparse.Namespace, rose_tree: RoseTree, cachedir: Optional[str], cwl_runner: str,
    ...
    if cwl_runner == 'toil-cwl-runner':
        ...
        cmd = ['toil-cwl-runner'] + net  # + provenance  # + docker_cmd_ +  path_check
        cmd += ['--outdir', 'outdir_toil',
                ...
                '--defaultMemory', '10GB',  # Not applied to the CWL jobs. The CWL CLTs should define RamMin in the ResourceRequirement section
                '--defaultDisk', '5Gi',
                # '--logDebug',
                ...
                # Using conda/docker/panassas NFS, so shouldn't need to deploy scripts.
                ...
                '--singularity',
                '--workDir', '/data1',  # "This directory needs to exist on all machines running jobs."
                # i.e. /run/user/$UID/coorddir  This is a local /tmpfs (in-memory) NOT NFS
                '--coordinationDir', '/data1',  # "Absolute path to directory where Toil will keep state and lock files."
                # '--log-dir', 'logdir',
                # '--logFile', 'logfile',
                # '--restart',
                '--disableCaching',
                ...
                '--cleanWorkDir', 'never',
                '--clean', 'never',
                # '--slurmArgs', '--export=ALL --partition=normal_gpu --error=serror.txt',
                '--relax-path-checks',  # permit spaces and hash characters
                # '--defaultCores', '2', # if you need more than 2 cpu cores
                # '--logLevel', 'DEBUG',  # for debugging
                '--batchLogsDir', './slurmlogs',  # save Slurm out and error files for each job
                f'autogenerated/{yaml_stem}.cwl', f'autogenerated/{yaml_stem}_inputs.yml']
        cmdline = ' '.join(cmd)
